...
def render_markdown(report: dict, *, out_path: str) -> None:
    ...
    Path(out_path).write_text(render_markdown_text(report), encoding="utf-8")


def render_markdown_text(report: dict) -> str:
    """Render a report to Markdown without writing to disk."""
...
def _build_lines(report: dict) -> list[str]:
    ...
    lines += [
        ...
        "",
        f"**Student degree:** {degree.get('student_degree', '')}  ",
        f"**JD title:** {degree.get('jd_title', '')}  ",
        f"**Suggested-title match:** {_tick(degree.get('title_on_suggested_list'))}  ",
        f"**Matched against:** {degree.get('matched_against', '')}  ",
