...
def analyse_degree_alignment(jd_profile: dict, degree_program: str) -> dict:
    """Compare the JD title with the suggested titles for a degree program."""
    user = (
        f"DEGREE PROGRAM: {degree_program}\n\n"
        f"JD PROFILE:\n{json.dumps(jd_profile, indent=2)}"
    )
    # Degree-title matching is a deterministic classification task; use the
    # wrapper's default temperature as specified by the Day 7 lab brief.
...
def summarise_overall(report: dict) -> str:
    ...
    return ask_text(OVERALL_SUMMARY_PROMPT, user, temperature=0.3, max_tokens=400)
...
def compute_overall_score(report: dict) -> int:
    ...
    """
    score = (
        report.get("keyword_match", {}).get("keyword_match_score", 0) * 0.40
        + report.get("bullets", {}).get("bullet_quality_avg", 0) * 0.25
        + report.get("structure", {}).get("structure_score", 0) * 0.15
        + report.get("jargon", {}).get("jargon_score", 0) * 0.10
        + report.get("degree_alignment", {}).get("degree_alignment_score", 0) * 0.10
    )
    return round(score)
