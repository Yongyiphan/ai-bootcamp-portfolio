...
def choose_next_action(state: dict, compile_result: dict, test_result: dict | None) -> tuple[str, str]:
    """Return the next bounded-loop action and its auditable reason."""
    if not compile_result["success"]:
        signature = "compile_failure"
    elif test_result and test_result["success"]:
        return "ACCEPT", "all_tests_passed"
    else:
        signature = test_result["failure_signature"] if test_result else "missing_test_result"
...
async def run_loop(mode: str, test_file: str, max_iterations: int) -> dict:
    ...
    while True:
        ...
        signature = (test_result["failure_signature"]
                     if test_result else "compile_failure")
